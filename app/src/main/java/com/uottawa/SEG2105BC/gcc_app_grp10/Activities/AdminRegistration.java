...
public class AdminRegistration extends AppCompatActivity implements Registration {
    EditText username ;
    EditText email;
    EditText password;
    EditText firstName;
    ...
    private boolean validateInputs(){
        ...
        if (username.getText().length() == 0) {
            Toast.makeText(AdminRegistration.this, "You need a username!", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (email.getText().length() == 0) {
            Toast.makeText(AdminRegistration.this, "You need an email!", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (!email.getText().toString().matches("^[a-zA-Z0-9_!#$%&’*+/=?`{|}~^.-]+@[a-zA-Z0-9.-]+$")) {
            Toast.makeText(AdminRegistration.this, "Invalid email address!", Toast.LENGTH_SHORT).show();
            return false;
        }
        if (password.getText().toString().length() < 6) {
            Toast.makeText(AdminRegistration.this, "Password must be at least 6 characters long!", Toast.LENGTH_SHORT).show();
            return false;
        }
